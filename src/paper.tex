...
Discretization in Target Speech Extraction (TSE) has seen limited study. SkiM-UniCATS \cite{gen_tse} is among the first to use discrete tokens in TSE, leveraging SSL models like HuBERT and vq-wav2vec \cite{vq_wav2vec}. However, it overlooks the WavLM model, which excels in speech separation. Also, it mainly focuses on single-layer outputs for discretization. Moreover, its evaluation is limited to speech quality, neglecting intelligibility and speaker similarity, both crucial for TSE. We address these gaps by proposing TSELM, which uses multiple WavLM layers for discretization and adds metrics for intelligibility and speaker similarity.
...
We follow the recipes\footnote{\href{https://github.com/xuchenglin28/speaker_extraction}{https://github.com/xuchenglin28/speaker\_extraction}} to from the reference speeches for 
WSJ0-2mix.
...
it hybrid because the mixture speech retains continuous features. In TSELM-S-NoCat, we utilize the TSELM-S model architecture but abandon concatenation strategy. 
...
\begin{table*}
  ...
  Spex+                   & D                                             & -                                                                                  & 3.38        & 3.77       & 3.00       & 19.0            & 0.922     &          & 3.49        & 4.00       & 3.21       & 15.0            & 0.943             \\
...
\end{center}
  ...
  \begin{table}
    \caption{Performance of different SSL models and layer selections on Libri2Mix clean. HuBERT model subsitutes the WavLM Large to HuBERT Large as the SSL model.
            The WavLM-L6 uses only the 6th layer of hidden output of WavLM Large. WavLM denotes our TSELM-L model which uses 6 hidden layers. 
            }
            \vspace{-20.5pt}
...
However, since discretization inherently results in some loss of speaker information—Target-Discrete shows a speaker similarity score of 0.653 for Libri2Mix compared to continuous methods—we assess the speaker similarity of the output from the discrete methods against the target audio produced by discretizing the clean speech. 
...
Although SkiM-UniCATS \cite{gen_tse} achieves satisfying DNSMOS scores, it does not ensure high speaker similarity, as indicated in Table \ref{main_exp}. Additionally, results in Table \ref{hubert_wavlm_6} show that it also does not guarantee superior speech intelligibility.
...
speaker similarity. However, we observe a gap between discrete methods and continuous methods especially in speech intelligibility and speaker similarity. Future research should focus on shrinking 
this gap. 
